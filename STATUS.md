...
最后更新：2026-09-05（§54：needs_resample 删除——33% 组曾整组跳过、37% sim 离散度被丢弃；全组参训，smoke 0-skip 全绿；§53 对拍/闸门设计见前）
...
## 54. needs_resample 删除:平坦组全额参训(2026-09-05)

- **裁决(用户)**:直接删除,不复辟——C1v8 塌方机制(σ-归一化把 0.01 噪声放大成 ±1 advantage)已随 v3.1 RAW 幅值死亡,skip 是那个时代的 belt-and-suspenders;多人场景不预期复现,真复现再说。
- **动机数据(run 2 groups.jsonl,344 组)**:flat 跳过 112(33%),其中 79% 全员零错;**被丢组的 sim_std(mean 0.0250)反而高于训练组(0.0210),61% 高于训练组中位,总 sim 离散度 37% 被丢弃**;跳过率偏斜 castorice 50%/hysilens 41%/cerydra 41%。悖论:内容全对组恰是学身份最安全处(advantage=纯 sim 零内容混淆),skip 专挑这种组不下课;试听证伪"SV 同策略排序=纯噪声"(身份差异可闻)。DAPO 跳组的正确类比如 reward 含 sim 则 advantage 非零,跳=从二元 reward 世界错误进口。
- **实现**:`grpo.py::needs_resample` 整删;`loop.py::_filter_trainable` → `_score_groups`(记账 + reward_v3,docstring 注明 needs_resample 删除日期),`skips` Counter 只剩 runaway/nonfinite_loss;`reward.py` docstring 引用移除;probes/tmp 两个 longest 探针的 import/调用改为无条件合成扰动;AGENTS invariants 两条更新。rollout/打分/groups.jsonl schema 零改动(flat_group 行此后不再出现,账本自带 ablation 对照)。
- **验证**:ruff 全绿 + import/CLI 过;GRPO 2 步 live smoke(runs/smoke54):**8 组 0 skipped(同 seed 旧基线 5+3skip),loss -0.0045/-0.002、KL 0→0.0023、grad 0.54/0.71、adv_std 0.066-0.089 全部有限健康**;平坦组以 sim-only advantage 参训(r_mos 0-0.003 地板照常);groups.jsonl 16 行全 skipped=false。边界情况:若 sim 恰好也全等 → 全组件熄火 → advantage≡0 零梯度空转,无害。
- **预期行为变化**:同条件下训练组 +53%(224→344);cipher/castorice/hysilens 的 flat 欠训自动缓解;最终裁决 = 下一轮正式 GRPO 的 E2 配对评测(留待用户排程)。
