...
ttm = LoraTrainerModel(
    "/home/felys/workspace/qwen3-tts-post-training/runs/sft_v1/export", device="cuda:1"
)
...
cache = pathlib.Path(
    "/home/felys/workspace/qwen3-tts-post-training/.cache/Chinese(PRC)"
)
...
codes = [
    torch.from_numpy(np.load(cache / "codes" / f"{r['name']}.npy")).long() for r in pick
]
...
print(
    f"ref log_probs |Δ| mean={d_mean:.6f} p99={d_p99:.4f} max={d_all.max().item():.4f} | mask max_abs = {d_m}"
)
print(
    f"widths: full {tuple(ref_full.log_probs.shape)} micro {tuple(ref_micro.log_probs.shape)}"
)
...
loss_f, met_f = grpo_loss(
    pol.log_probs, ref_full.log_probs, R, pol.mask, group_ids, cfg
)
zero_grads()
(loss_f).backward()
...
print(
    f"full: loss={loss_f.item():.6f} kl={met_f.kl.item():.6f} W={W_total.item():.1f} grad_norm={norm_f:.4f}"
)
...
for i in range(0, gs, 4):
    ...
    w = pol_c.log_probs.shape[1]
    pad = ref_full.log_probs.shape[1] - w
    ...
    loss_c, met_c = grpo_loss(
        lp_c, ref_full.log_probs[sl], A[sl], mk_c, group_ids[sl], cfg, advantage=A[sl]
    )
...
rel = max(
    ((g_full[n] - g_micro[n]).abs().max() / gscale).item()
    for n in g_full
    if n in g_micro
)
...
print(
    "PASS"
    if d_mean < 0.02 and rel < 0.05 and abs(loss_f.item() - loss_m) < 5e-3
    else "FAIL"
)
