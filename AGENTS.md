...
- Sampler impls (`--sampler-impl`, PROJECT_STATUS §9): `hf` (reference), `fast` (hand-rolled loop in `fastgen.py`, bit-equal vs `hf` incl. the RNG stream — verified greedy + seeded), `compiled` (same loop + `torch.compile(dynamic=None, epilogue_fusion=False)` on the two backbone forwards, ~2.4x), `graphed` (CUDA-graph decode in `fastgraph.py`, ~4.8x: prefill eager into a static KV pool, then one captured 1-token step per replay; `flash_attn_with_kvcache` attention, fixed batch = group_size, other batch sizes fall back to eager fastgen).
- `compiled` numerically drifts vs eager (inductor float reassociation; greedy argmax can flip at step 0) — bit-equality with `hf` does NOT hold; its contract is bitwise self-reproducibility after the built-in warmup (two dummy generations in `FastSampler.__init__` absorb dynamo's static->dynamic promotion). Do not remove the warmup and do not assume a fresh cold process reproduces a warm process's rollout.
...
- Self-reproducibility: same seed, run twice, `torch.equal` on every output tensor (built into `GraphFastSampler._warmup_graph`). Catches nondeterminism and state leakage between calls.
