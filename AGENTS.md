...
- Sampler impls (`--sampler-impl`, PROJECT_STATUS §9; all under `workers/trainer/src/trainer/samplers/`, ABC = `base.Sampler`, factory = `build_sampler`): `hf` (reference, `hf.py`, shared `base.prefill_cur_len`), `fast` (hand-rolled eager loop in `eager.py`, `EagerSampler`, bit-equal vs `hf` incl. RNG — verified greedy+seeded), `compiled` (`torch.compile(dynamic=None,epilogue_fusion=False)` on two backbones, `torch_compile.py`, ~2.4x, fixed batch), `graphed` (CUDA-graph decode `cuda_graph.py`, ~4.8x: prefill eager into static `KV lmax=token_budget_infer` pool then 1-token graph replay; `flash_attn_with_kvcache`, fixed batch). All `sample` return `tuple[codes, cur_len]` where `cur_len=mask.shape[1]` and `max_new = token_budget - cur_len` (dual `token_budget 512` train / `token_budget_infer 1024` infer replaces `max_new_tokens/lmax/runaway_t_max`). `Decoder.decode` is `@torch.inference_mode`.
