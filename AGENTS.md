...
- `workers/trainer/` — GRPO trainer worker (default `cuda:1`). Entry: `workers/trainer/main.py`. Package layout: shared kernels at `src/trainer/` top level (`batch.py` = official collate + tokenize, `model.py` = TrainerModel + `teacher_forcing` kernel), backend code under `src/trainer/grpo/` (loop/logprob/rollout/decoder/samplers) and `src/trainer/sft/` (planned entry + loss).
